                ...
                @forelse($recentOrders as $order)
                    @php
                        ...
                        if (strlen($productNames) > 40) {
                            $productNames = substr($productNames, 0, 37) . '...';
                        }
                    @endphp
                    <tr>
                        <td><span style="font-family:var(--mono); font-size:12px; color:var(--accent); font-weight:600;">#{{ $order->order_id }}</span></td>
                        <td>{{ $productNames }}</td>
                        <td style="font-family:var(--mono); font-weight:700;">{{ number_format($order->total_amount, 2) }}</td>
                        <td><span class="pill pill-{{ $order->order_status }}">{{ ucfirst($order->order_status) }}</span></td>
                    </tr>
                @empty
                    <tr>
