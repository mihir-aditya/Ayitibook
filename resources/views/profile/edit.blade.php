...
select[name="dial_code"] option {
    color: #000 !important;
    background-color: #dbc9c9 !important;
    -webkit-text-fill-color: #000 !important; /* Safari fix */
}

/* Also fix the gender select similarly */
select[name="gender"],
...
@include('includes.header')
 <!-- Sidebar -->
  ...
  <div class="profile-pic-wrapper text-center mb-4 position-relative">
    <img src="{{ $user->profile_pic ? asset('storage/'.$user->profile_pic) : asset('assets/images/default.jpg') }}" id="profilePic" class="rounded-circle" alt="Profile Picture">
    ...
    <button type="button" id="deletePic" class="btn btn-danger btn-sm mt-2 {{ $user->profile_pic ? '' : 'd-none' }}">Delete</button>
  </div>
    ...
    <form method="POST" action="{{ route('profile.account') }}" enctype="multipart/form-data">
      ...
      <div class="row g-3 mb-4">
        ...
        <div class="col-md-3 position-relative">
          ...
          <input type="text" name="name" class="form-control" value="{{ old('name', $user->name ?? ($user->first_name . ' ' . $user->last_name) ?? '') }}" required />
          <span class="edit-icon" style="position:absolute; right:10px; top:38px; cursor:pointer;">&#9998;</span>
        </div>
        <div class="col-md-3 position-relative">
         ...
         <div class="col-md-3 position-relative">
           ...
           <select class="form-select" name="gender">
             ...
             <option value="not_say" {{ old('gender', $profile->gender ?? '') === 'not_say' ? 'selected' : '' }}>Prefer not to say</option>
           </select>
         </div>
        ...
        style="max-width:75px; color:#000 !important; background-color:#dbc9c9 !important; -webkit-appearance:auto;">
                <option value="+91" {{ old('dial_code', $profile->dial_code ?? '+1') === '+91' ? 'selected' : '' }}>+91</option>
                <option value="+1" {{ old('dial_code', $profile->dial_code ?? '+1') === '+1' ? 'selected' : '' }}>+1</option>
                <option value="+44" {{ old('dial_code', $profile->dial_code ?? '+1') === '+44' ? 'selected' : '' }}>+44</option>
                <option value="+61" {{ old('dial_code', $profile->dial_code ?? '+1') === '+61' ? 'selected' : '' }}>+61</option>
              </select>
              <input type="text" name="phone" class="form-control" value="{{ old('phone', $user->phone ?? '') }}" placeholder="Enter your Number" required />
            </div>
          <span class="edit-icon" style="position:absolute; right:10px; top:38px; cursor:pointer;">&#9998;</span>
        </div>
