  ...
  <div class="row">
    ...
    <div class="col-lg-9">
      <div class="cancel-card">
        <h3 class="mb-3">Canceled Orders</h3>
                 <!-- Search + Filter row -->
        ...
        <div class="table-responsive">
          <table class="table align-middle">
            ...
            <tbody>
              ...
              <tr>
                <td>{{ $order->id }}</td>
                <td><img src="{{ $order->items->first()?->product?->image ?? '../assets/images/default.png' }}" class="product-img" alt=""></td>
                <td>{{ $order->items->first()?->product?->name ?? 'N/A' }}</td>
                <td>${{ number_format($order->total_amount, 2) }}</td>
                <td><span class="status-badge cancelled">Cancelled</span></td>
                <td>{{ $order->cancellation_reason ?? 'N/A' }}</td>
                <td>{{ $order->payment_method ?? 'N/A' }}</td>
                <td>{{ $order->placed_at ? $order->placed_at->format('M d, Y') : 'N/A' }}<br>{{ $order->placed_at ? $order->placed_at->format('h:i A') : '' }}</td>
                <td>
                    <a href="#" class="btn btn-sm btn-outline-primary me-2"> <i class="bi bi-pencil-square me-1"></i> Write Review</a>
                    <a href="#" class="btn btn-sm btn-outline-success"> <i class="bi bi-cart-plus me-1"></i> Buy Now</a>
